        ...
        <>
            {
                (photos.length > 0) ? (
                    photos.map(photo => {
                        return (
                            <div key={photo.id} className="relative bg-gray-300 w-fit h-fit mb-4 mx-auto">
                                <Image width={400} height={600} src={`${photo.urls.regular}&fm=jpg&fit=max`} alt={photo.user.username} className="opacity-90 object-cover md:aspect-square" />
                                <a
                    ...
                    <div className="w-full h-full flex flex-col text-center items-center col-span-3 row-span-2">
