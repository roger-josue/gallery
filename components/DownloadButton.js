...
export default function DownloadButton({ location, filename = 'image.jpeg' }) {
    ...
    return (
        (location) &&
        <motion.button transition={{duration: 1}} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} onClick={handleClick} disabled={!toggleButton} className="absolute text-lg bottom-1 right-2 transition-all duration-500 shadow-md text-darkText p-2 rounded-md hover:bg-primary disabled:bg-secondary">
