...
export default function ModalMenu({ toggleModal, setToggleModal, photo }) {
    ...
    const handleExit = ({ type, code, target }) => {
        if (type === "keydown" && code === "Escape" || type === "click" && target.localName === "dialog")
            setToggleModal(false);
    }


    return (
        <motion.dialog initial={{opacity: 0}} transition={{duration: 0.5}} whileInView={{opacity: 1}} viewport={{once: true}} onKeyDown={(e) => handleExit(e)} onClick={(e) => handleExit(e)} ref={modal} className="fixed z-20 inset-0 w-screen h-screen bg-gray-800 bg-opacity-70 flex flex-col place-content-center items-center p-20 cursor-zoom-out">
            <motion.div transition={{ duration: 1 }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="relative w-[90vw] h-[90vh] sm:w-[60vw] sm:h-[80vh] lg:w-[50vw] lg:h-[70vh] cursor-default">
